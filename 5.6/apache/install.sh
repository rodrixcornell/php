...
&& apt-get install -yq --no-install-recommends php$PHP_VERSION-{bcmath,curl,gd,json,mbstring,mcrypt,mysql,sqlite*,xml,zip} apache2 libapache2-mod-php$PHP_VERSION libaio1 curl alien make php$PHP_VERSION-dev \
...
\
&& cd ../pdo_oci \
&& cd /usr/include/php/ && ln -sfv 201*/ext ext && cd - \
...
\
&& cd && apt-get purge -y curl alien gcc-7-base make php$PHP_VERSION-dev oracle-instantclient$ORACLE_INSTANTCLIENT-devel locales software-properties-common && apt-get autoclean && apt-get autoremove -y && rm -rf /var/lib/apt/lists/* && rm -rf /usr/include/php /tmp/php* /tmp/oracle* /usr/lib/python3
